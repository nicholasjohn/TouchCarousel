(function () {
  ...
  */
  ...
  $('.js-hero-carousel .hero-thumb').each(function(i,v){
    var $currentSlideimg = $(this).find('img')
      , $a   = $(this).children('a')
      , $h   = $(this).find('h3')
      , $meta = $(this).find('.meta')
      , $newImg = $('<img>').attr({
        src: $currentSlideimg.data('hero-url'),
    ...
    $(this).addClass('js-hero-thumb').attr('data-slide',(i+1)).prepend($timerDiv)
  ...
  function goToSlide($slideNum){
    ...
    $('.hero-thumb.active').removeClass('active')
    $('.animating').removeClass('animating')
    $('.hero-thumb[data-slide="'+$slideNum+'"]').addClass('active').find('.js-reel-timer-overlay').addClass('animating')
  ...
  */
  ...
  .on('exit.two-col enter.two-col', function () {
    $subWidth = $('.js-hero-carousel .hero-thumb:first-child').width()
  ...
  $heroReel.on('touchstart', function(e){
    ...
    $(this).on( 'touchmove', function(e){
      ...
      if (Math.abs($currentX - $initialX) >
        ...
        // add active state
        $('.js-hero-thumb.active').find('.js-reel-timer-overlay').removeClass('animating').addClass('full')
    ...
    $(this).on('touchend', function(e){
      ...
      $('.js-hero-thumb.active').find('.full').removeClass('full').width(0).addClass('animating')
